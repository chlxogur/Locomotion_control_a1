...
class BasePositionSensor(sensor.BoxSpaceSensor):
  ...
  def _get_observation(self) -> _ARRAY:
    return np.array(self._robot.GetBasePosition()[:2])
...
class TargetSensor(sensor.BoxSpaceSensor):
  
  def __init__(self,
               lower_bound: _FLOAT_OR_ARRAY = -100,
               upper_bound: _FLOAT_OR_ARRAY = 100,
               name: typing.Text = "TargetVector",
               dtype: typing.Type[typing.Any] = np.float64) -> None:

    super(TargetSensor, self).__init__(
        name=name,
        shape=(3,),  # x, y, angle
        lower_bound=lower_bound,
        upper_bound=upper_bound,
    ...
    self.current_target_x = 4.
    self.current_target_y = 0.

  def _get_observation(self) -> _ARRAY:
    _, _, yaw = self._robot.GetTrueBaseRollPitchYaw()
    rot_mat = np.array([[np.cos(yaw), np.sin(yaw)],
                        [-np.sin(yaw), np.cos(yaw)]])

    target_vector = np.array([self.current_target_x, self.current_target_y]) - np.array(self._robot.GetBasePosition()[:2]) 
    robot_base = rot_mat @ target_vector
    #robot_base_norm = robot_base / np.linalg.norm(robot_base)
    #target_vector_norm = target_vector / np.linalg.norm(target_vector)
    #head_vector = np.array([math.cos(yaw), math.sin(yaw)])
    #angle_diff = math.acos(np.dot(target_vector_norm, head_vector))
    return np.array([robot_base[0], robot_base[1], yaw])
  
  def on_step(self, env):
    self.current_target_x = env.target_x
    self.current_target_y = env.target_y
