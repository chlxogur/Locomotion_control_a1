...
class LocomotionGymEnv(gym.Env):
  ...
  def step(self, action):
    ...
    if self.init_txt:
        self.debug_txt=self._pybullet_client.addUserDebugText("GOAL", [self.target_x, self.target_y, 0.1], textColorRGB=[1,0,0], textSize=1.5)
        self.init_txt = False

    if self.goal == True:
      self._pybullet_client.removeUserDebugItem(self.debug_txt)
      r = 4
      theta =  np.pi / 6
      self.target_x = r * math.cos(theta * self.target_cnt)
      self.target_y = r * math.sin(theta * self.target_cnt)
      self.debug_txt=self._pybullet_client.addUserDebugText("GOAL", [self.target_x, self.target_y, 0.1], textColorRGB=[1,0,0], textSize=1.5)
      self.goal = False
      self.target_cnt = self.target_cnt + 1
      
      #print('target_changed : ' + str(self.target_x) + ', ' + str(self.target_y))
